...
public class CupHolder2810 {
	...
	*S*LL*LL*S*S*LL*
	...
	public static void main(String[] args) throws Exception{
		...
		int input = Integer.parseInt(br.readLine());//총인원수
		String seat = br.readLine();//좌석 입력
		
		seat = seat.replace("LL", "X");//지환
		
		int possibleHolder = seat.length() + 1;//컵 사용가능한 공간
		
		if (input <= possibleHolder) {
			bw.write(input +  "");
			
		} else {
			
			bw.write(possibleHolder + "");
