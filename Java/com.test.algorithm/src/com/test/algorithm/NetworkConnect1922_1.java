...
public class NetworkConnect1922_1 {
	...
	public static void main(String[] args) throws Exception {
		...
		Collections.sort(nodeArr,new ListSort());
		
		
		for (int i = 0; i < M; i++) {
			if (count < N- 1) {
				int x1 = nodeArr.get(i).node1;
				int x2 = nodeArr.get(i).node2;
				int weight = nodeArr.get(i).weight;
				
				if (equalsParent(x1,x2) == 0) {
					unionParent(x1,x2);
					maxWeight += weight;
				}
			}
	...
	static int getParent(int x) {
		...
		else return getParent(parents[x]);
	}
