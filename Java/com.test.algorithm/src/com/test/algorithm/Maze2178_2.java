...
public class Maze2178_2 {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < N; i++) {
			String[] dataInputs = br.readLine().split("");
			for (int j = 0; j < M; j++) {
				matrix[i][j] = Integer.parseInt(dataInputs[j]);
			}
		}
	...
	static void bfs(int x, int y) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (newY < 0 || newX < 0 || newY >= M || newX >= N || matrix[newX][newY] == 0 || visit[newX][newY] != 0)
				continue;
