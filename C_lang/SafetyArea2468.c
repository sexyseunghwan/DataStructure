...
int matrix[101][101];
int flag_matrix[101][101];
int dx[4] = {0,0,1,-1};
int dy[4] = {1,-1,0,0};
int N,water;

void dfs(int x,int y);
void clear_matrix(int N);
...
int main(){
    ...
    for (int i = min_num; i <= max_num; i++) {
        ...
        for (int j = 0; j < N; j++) {
            for (int k = 0; k < N; k++) {
                if (flag_matrix[j][k] == 0 && matrix[j][k] > water) {
                    dfs(j,k);
                    count++;
                }
            }
...
void dfs(int x, int y) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (new_x < 0 || new_y < 0 || new_x >= N || new_y >= N || flag_matrix[new_x][new_y] == 1 || matrix[new_x][new_y] <= water) {
            continue;
        }
        dfs(new_x,new_y);
    }
